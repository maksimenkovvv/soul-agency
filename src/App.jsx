...
function AppSeo() {
  ...
  const SITE_NAME = readEnvCra('REACT_APP_SITE_NAME', 'Бюро Души');
  const SITE_URL = readEnvCra('REACT_APP_SITE_URL', '');
  const DEFAULT_DESCRIPTION = readEnvCra('REACT_APP_SITE_DESCRIPTION', 'Платформа для подбора психолога, записей на сессии и общения.');
  ...
  const websiteSchema = useMemo(() => {
    if (!siteOrigin) return null;
    const searchTarget = new URL('/blog?query={search_term_string}', siteOrigin).toString();
  ...
  return (
    <Helmet prioritizeSeoTags>
      ...
      {orgSchema ? <script type="application/ld+json">{JSON.stringify(orgSchema)}</script> : null}
      {websiteSchema ? <script type="application/ld+json">{JSON.stringify(websiteSchema)}</script> : null}
