...
export default function OurPsychologists({
  ...
  const openModal = useCallback((p, extra = {}) => {
    ...
    const modalData = {
      ...
      sessionDuration:
        raw?.sessionDurationMinutes ?? raw?.session_duration_minutes ?? raw?.sessionDuration ?? 50,
  ...
  // Синхронизация GSAP после загрузки данных или подгрузки "Показать ещё"
  useEffect(() => {
    // Ждем, пока loading или loadingMore станут false
    if (!loading && !loadingMore) {
      // setTimeout(..., 200) нужен, чтобы React успел реально отрисовать DOM,
      // а браузер успел пересчитать высоту блоков
  ...
  }, [loading, loadingMore, items]); // Следим за загрузкой и самим списком
