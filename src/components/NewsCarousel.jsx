        ...
        >
            <div className="news-carousel__head">
                ...
                <div className="news-carousel__actions">
                    <button type="button" className="news-carousel__nav" onClick={() => scrollByCards(-1)} aria-label="Назад">
                        ‹
                    </button>
                    <button type="button" className="news-carousel__nav" onClick={() => scrollByCards(1)} aria-label="Вперёд">
                        ›
                    </button>
                    <Link to="/blog" className="b-btn b-btn--transparent news-carousel__all">Все статьи</Link>
                </div>
            </div>

            <div ref={railRef} className="news-carousel__rail" tabIndex={0}>
                {loading ? (
                    ...
                    items.map((p) => (
                        <Link key={p.slug || p.id} to={`/blog/${p.id}-${p.slug}`} className="news-card">
                            <div className="news-card__media">
                                {p.previewImageUrl ? <img src={p.previewImageUrl} alt={p.title || 'Обложка'} /> : null}
                            </div>
